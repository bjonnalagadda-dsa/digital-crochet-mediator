 ...
 */
function parseStepCounters(text: string): {
  round: number | null;
...
const STITCH_LIBRARY: { [key: string]: StitchInfo } = {
  ...
  dc: {
    ...
    definition:
      "Yarn over, insert hook, pull up loop, yarn over pull through 2, yarn over pull through 2.",
    videoUrl:
  ...
  hdc: {
    ...
    definition:
      "Yarn over, insert hook, pull up loop, yarn over, pull through all 3 loops.",
    videoUrl:
  ...
  "sl st": {
    ...
    definition:
      "Insert hook, yarn over, pull through stitch and loop on hook in one motion.",
    videoUrl:
  ...
  tr: {
    ...
    definition:
      "Yarn over twice, insert hook, pull up loop, then yarn over and pull through 2 loops three times.",
    videoUrl:
      "https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/Sintel.mp4",
  },
  rep: {
  ...
  inc: {
    ...
    title: "Increase (inc)",
    definition:
      "Work two stitches into the same stitch to increase stitch count.",
    videoUrl:
      "https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/Sintel.mp4",
  },
  dec: {
  ...
  tog: {
    ...
    title: "Together (tog)",
    definition:
      "Work stitches together into one (used in decreases like hdc2tog).",
    videoUrl:
      "https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/Sintel.mp4",
  },
  round: {
...
/** "repeat twice" → "3× total (once + repeat 2 more times)" */
function clarifyRepeatCount(raw: string): string {
  const lower = raw.toLowerCase().trim();
  for (const [word, n] of Object.entries(WORD_TO_NUM)) {
    if (lower.includes(word)) {
      const total = n + 1;
      return `${total}× total — do once, then repeat ${n === 2 ? "twice" : `${n} more times`}`;
    }
  }
  ...
  if (numMatch) {
    ...
    const total = n + 1;
    return `${total}× total — do once, then repeat ${n} more times`;
  }
  return raw.replace(/^repeat\s*/i, "Repeat ");
}
...
type StepSegment =
  ...
  | { kind: "repeat"; body: string; count: string }
  | { kind: "note"; text: string };

/** Split text at top-level commas (ignores commas inside parentheses) */
function splitAtCommas(text: string): string[] {
  const parts: string[] = [];
  let depth = 0;
  let current = "";
...
/** Parse step body into structured segments (instructions, repeat blocks, notes) */
function parseSegments(body: string): StepSegment[] {
  ...
  while (i <= groups.length) {
    const g = groups[i];
    const plainText = g ? body.slice(pos, g.start) : body.slice(pos);
    // Emit instruction sub-lines from plain text
    splitAtCommas(plainText).forEach((chunk) => {
      if (chunk.trim())
        segments.push({ kind: "instruction", text: chunk.trim() });
    });
    if (!g) break;

    const inner = g.inner;
    if (/^repeat/i.test(inner)) {
      // Standalone repeat count — attach to previous repeat segment if present
      const last = segments[segments.length - 1];
      if (last && last.kind === "repeat" && !last.count) {
        last.count = inner;
      }
    } else if (/counts as/i.test(inner)) {
      segments.push({ kind: "note", text: inner });
    } else {
      // Check if the immediately following group is a repeat count
      const next = groups[i + 1];
      if (next && /^repeat/i.test(next.inner)) {
        segments.push({ kind: "repeat", body: inner, count: next.inner });
        pos = next.end;
        i += 2;
        continue;
      }
      segments.push({ kind: "repeat", body: inner, count: "" });
    }
    pos = g.end;
    i++;
  }
...
/** Render a string with crochet term highlighting */
function renderTokens(
  text: string,
  knownTerms: Set<string>,
  onTerm: (t: string) => void,
  getMasteryLevel: (t: string) => MasteryLevel,
  extraStyle?: object,
) {
    ...
    if (norm && knownTerms.has(norm)) {
      ...
      return (
        <Text
          key={idx}
          onPress={() => onTerm(norm)}
          style={[styles.termInline, MASTERY_TERM[level]]}
        >
          {token}
...
function StepLine({
  ...
  // Separate the "Row N" header from the instruction body
  const headerMatch = step.text.match(
    /^((?:row|round|rnd)\s*\d+)\s*[-–—]?\s*/i,
  );
  const header = headerMatch ? headerMatch[1] : `Step ${index + 1}`;
  const body = headerMatch ? step.text.slice(headerMatch[0].length) : step.text;
  ...
  return (
    ...
    >
      ...
      <View style={{ flex: 1 }}>
        {/* Row label */}
        <Text
          style={[styles.stepHeader, isCurrent && styles.stepHeaderCurrent]}
        >
          {header.toUpperCase()}
        </Text>

        {segments.map((seg, si) => {
          if (seg.kind === "instruction") {
            return (
              <Text
                key={si}
                style={[
                  styles.stepSubLine,
                  isCurrent && styles.stepSubLineCurrent,
                ]}
              >
                {"• "}
                {renderTokens(seg.text, knownTerms, onTerm, getMasteryLevel)}
              </Text>
            );
          }
          if (seg.kind === "repeat") {
            return (
              <View key={si} style={styles.repeatBlock}>
                <View style={styles.repeatHeaderRow}>
                  <Text style={styles.repeatIcon}>↻</Text>
                  <Text style={styles.repeatLabel}>
                    {seg.count
                      ? clarifyRepeatCount(seg.count)
                      : "Repeat section"}
                  </Text>
                </View>
                <Text style={styles.repeatBody}>
                  {renderTokens(seg.body, knownTerms, onTerm, getMasteryLevel, {
          ...
          if (seg.kind === "note") {
            return (
              <Text key={si} style={styles.stepNote}>
                ℹ {seg.text}
              </Text>
            );
          }
...
export default function HomeScreen() {
  ...
  useEffect(() => {
    ...
    const offset = stepRowHeights.current
  ...
  useEffect(() => {
    ...
    const { round, stitches } = parseStepCounters(
      s[currentStepIndex]?.text ?? "",
    );
  ...
  const openHelp = useCallback(
    (key: string) => {
      ...
      const item: StitchInfo = backendStitches[norm] ??
        STITCH_LIBRARY[norm] ?? {
          key: norm,
    ...
    <SafeAreaView style={styles.root} edges={["bottom", "left", "right"]}>
      ...
      <View style={[styles.header, { paddingTop: insets.top }]}>
        ...
        <View style={styles.headerActions}>
          <Pressable
            ...
            onPress={uploadPdf}
          >
            <Text style={[styles.btnText, { color: "#0b1220" }]}>
              Upload PDF
            </Text>
          </Pressable>
          {pdfUri && (
      ...
      {!pdfUri ? (
        ...
        <View style={styles.empty}>
          <Text style={styles.emptyTitle}>No pattern selected</Text>
          <Text style={styles.emptyBody}>
            Tap <Text style={styles.emptyHighlight}>Upload PDF</Text> to add a
            crochet pattern from your device. Crochet terms in the pattern will
            be highlighted — tap any to see a definition and tutorial video.
          </Text>
        ...
        <View style={styles.body}>
          ...
          <View style={styles.viewToggle}>
            <Pressable
              style={[
                styles.toggleBtn,
                activeView === "pdf" && styles.toggleBtnActive,
              ]}
            ...
            >
              <Text
                style={[
                  ...
                  activeView === "pdf" && styles.toggleBtnTextActive,
                ]}
              >
                PDF Document
              </Text>
            ...
            <Pressable
              style={[
                styles.toggleBtn,
                activeView === "steps" && styles.toggleBtnActive,
              ]}
            ...
            >
              <Text
                style={[
                  styles.toggleBtnText,
                  activeView === "steps" && styles.toggleBtnTextActive,
                ]}
              >
                Pattern Steps
              </Text>
            ...
            <View style={styles.fullView}>
              {analysing ? (
                ...
                <>
                  ...
                  <View style={styles.progressHeader}>
                    ...
                    <Text style={styles.stepsHint}>
                      Tap any{" "}
                      <Text style={styles.termInlineStatic}>
                        highlighted term
                      </Text>{" "}
                      for its definition and tutorial.
                    </Text>
                  </View>
                    ...
                    <View style={styles.counterCard}>
                      ...
                      <View style={styles.counterControls}>
                        <Pressable
                          style={styles.counterBtn}
                          onPress={() =>
                            setStitchCount(Math.max(0, stitchCount - 1))
                          }
                        >
                          <Text style={styles.counterBtnText}>−</Text>
                        </Pressable>
                        ...
                        <Pressable
                          style={styles.counterBtn}
                          onPress={() => setStitchCount(stitchCount + 1)}
                        >
                          <Text style={styles.counterBtnText}>+</Text>
                        </Pressable>
                      </View>
                  ...
                  <View style={styles.navBar}>
                    ...
                    <Pressable
                      style={[
                        styles.micBtn,
                        wizConnected && styles.micBtnActive,
                      ]}
                      onPress={toggleWiz}
                    >
                      <Ionicons
                    ...
                    >
                      <Text style={[styles.navBtnText, { color: "#0b1220" }]}>
                        Next →
                      </Text>
                    </Pressable>
                  </View>
                  ...
                  <View style={styles.voiceStrip}>
                    {wizConnected ? (
                      ...
                      <Text style={styles.voiceErrorText}>
                        Microphone unavailable — tap mic to retry
                      </Text>
                    ) : (
                      <Text style={styles.voiceStatusText}>
                        Tap the mic to enable voice navigation
                      </Text>
                    )}
                  </View>
                </>
                ...
                <ScrollView contentContainerStyle={styles.stepsPadding}>
                  ...
                  <Text style={styles.stepsHint}>
                    No step-by-step breakdown was found for this pattern, but
                    these crochet terms were detected. Tap any to see its
                    definition and tutorial video.
                  </Text>
                ...
                <View style={styles.centered}>
                  ...
                  <Text style={styles.noStepsBody}>
                    The backend didn't find structured steps in this pattern.
                    Try viewing the PDF directly.
                  </Text>
                </View>
              )}
            </View>
            ...
            <>
              ...
              {selected.videoUrl ? (
                ...
                <Text style={styles.noVideo}>
                  No tutorial video available for this term yet.
                </Text>
              )}
            </>
          ) : (
            <Text style={styles.sheetDef}>
              Tap a highlighted stitch term to see its definition and tutorial.
            </Text>
          )}
...
const styles = StyleSheet.create({
  ...
  stepSubLineCurrent: { color: "white" },
  stepNote: {
  ...
  repeatIcon: { color: "#ffd166", fontSize: 14 },
  repeatLabel: {
    color: "#ffd166",
    fontWeight: "800",
    fontSize: 11,
  ...
  repeatBody: { color: "rgba(255,255,255,0.82)", fontSize: 13, lineHeight: 20 },
